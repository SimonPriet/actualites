...
public class InfoControllerHelper extends BaseExtractorHelper {
	...
	public void listLastPublishedInfos(final HttpServerRequest request) {
		...
		extractUserFromRequest(thread, request, new Handler<BaseResource>() {
			...
			public void handle(final BaseResource model) {
				try {
					RequestUtils.bodyToJson(request, pathPrefix + "listLastPublishedInfos", new Handler<JsonObject>() {
						@Override
						public void handle(final JsonObject object) {

							int resultSize = object.getInteger("result_size", 0);
							if(resultSize == 0) {
								badRequest(request);
								return;
							}

							infoService.listLastPublishedInfos(thread.getUser(), resultSize, new Handler<Either<String, JsonObject>>() {
								@Override
